...
export const copyFormatBrush = function () {
...
const setCopyFormatting = (quill, copyFormatting, value, format) => {
...
const pasteFormat = (range, oldRange, source, quill, copyFormatting) => {
  if (range && copyFormatting.format) {
    if (range.length === 0) {
    } else {
      // quill.formatText(range.index, range.length + 1, copyFormatting.format);
      quill.formatText(range.index, range.length, copyFormatting.format);
